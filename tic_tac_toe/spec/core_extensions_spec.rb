...
describe Array do
  ...
  context "#any_empty?" do
    ...
    it "returns false if none of the Array elements are empty" do
      expect(["aasgasga", 1, "gegeagaeg", Object.new, :a].any_empty?).to be_false
    end
  ...
  context "#none_empty?" do
    it "returns true if none elements of the Array are empty" do
      expect(["A", "B", "A"].any_empty?).to be_true
    end
   
    it "returns false if some of the Array elements are not empty" do
      expect(["aasgasga", 1, "", Object.new, :a].any_empty?).to be_false
    end
   
    it "returns false for an empty Array" do
      expect([].all_empty?).to be_false
    end
  end
